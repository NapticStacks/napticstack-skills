---
name: naptic-sprint-recap
description: Generate a 3-paragraph plain-English recap of what shipped in the current session at a 7th-grade reading level — what got built, how each piece works, how it fits the grand scheme. Use proactively at the end of every sprint, push, /ship workflow, /land-and-deploy, or multi-PR session. Also use on demand when the user asks to "recap", "summarize the push", "explain what we built", "kid-friendly summary", or "ELI7".
---

# naptic-sprint-recap

Naptic-owned skill for closing out a push with language the operator can paste directly into a client conversation, an investor update, or a stand-up. Three paragraphs, 7th-grade reading level, no jargon, anchored to the human who benefits.
...
If unsure: lean toward invoking. Cost is low; benefit is the operator gets words they can paste to a non-technical audience immediately, without translating from PR titles or commit messages.
...
**Avoid these AI-vocabulary words** (gstack voice rules):
...
3. TodoWrite / TaskList tasks marked completed this session
...
This skill is one of the few napticstack-skills that does NOT wrap a gstack skill — there's no upstream `gstack /sprint-recap` to defer to. It's a standalone Naptic addition. The operator runs it directly; no underlying skill invocation.
